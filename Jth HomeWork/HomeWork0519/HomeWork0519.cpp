...
void TrimDelete(char* _string)
{
    ...
    while (_string[readIndex])
    {
        char Ch = _string[readIndex];

        //while ()
       //{
        if (Ch != ' ')
        {
            _string[writeIndex] = Ch;
            ++writeIndex;
...
int main()
{
    ...
    {
        // 8이 리턴되게 만들어라.
        int Result = DigitsCount(34454546);
...

    {
        ...
        std::cout << "ArrCopyText: " << ArrCopyText << std::endl;
